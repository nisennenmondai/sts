...
static void _encrypt_aes_ecb(mbedtls_aes_context *ctx, unsigned char *input, 
...
static void _decrypt_aes_ecb(mbedtls_aes_context *ctx, unsigned char *input, 
                unsigned char *output, size_t size)
{
        int i;
        int iter;
...
int sts_ecdh_aes_test(char **argv)
{
        ...
        if (argv[1] == NULL) {
                ERROR("sts: wrong number of arguments -> sectest [MSG]\n");
                return STS_PROMPT;
        }
        char *msg = argv[1];
        int msg_size = 0;
        ...
        _encrypt_aes_ecb(&host_aes_ctx, message, enc_msg, msg_size);
        _decrypt_aes_ecb(&remote_aes_ctx, enc_msg, dec_msg, msg_size);
